...
2. **提供视觉伴侣**（如果主题涉及视觉问题）— 这是单独的消息，不与澄清问题合并。参见下文的视觉伴侣部分（即 craft-studio）。
...
## 视觉伙伴
**重要提示**: 详细使用`skill` 加载:
`zjkycode/craft-studio`

基于浏览器的伙伴（craft-studio = 反 AI 味设计 intelligence），用于在头脑风暴期间展示模型、图表和视觉选项。意味着它可用于从视觉处理中受益的问题；这并不意味着每个问题都通过浏览器进行。
